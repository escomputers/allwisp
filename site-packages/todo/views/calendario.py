...
"""
#Vecchio metodo con variable di sessione
@login_required
@user_passes_test(staff_check)	
def get_data_con_orario(request):
	#Get ajax variable that tell us if dates contain time or not
	if request.method == 'POST' and 'data' in request.POST:
		ajax_response_dict = json.loads(request.POST['data'])
		ajax_date = ajax_response_dict.get('date_id')
		
		#Make variable a session variable in order to be read from other functions inside this file
		request.session['dato'] = ajax_date 
...
def calendario(request):
	...
	#Check if date contains time or not
	...
	if notify_request:
		...
		if isRecur:
			...
			#Dates contain time
				...
				print('ciao da evento ricorrente con orario specificato')
				
		#If event is not recurrent			
		else:
				#And date doesn't contain time
				if dato == '0':
				...
				#Date contains time
					...
					print('ciao da evento NON ricorrente con orario specificato')
