...
def controllo_id(request):
	if request.is_ajax():
		idinserito=request.POST.get("customer_id")
		response = {
		    'is_taken': Customer.objects.filter(customer_id__iexact=idinserito).exists()
		}
...
def nuovocliente(request):
	...
	if clienti.is_valid():
		clienti.save()
		clienti=clientiForm()
		return render(request, 'nuovocliente.html',{"clienti":clienti, "interessato":interessato, 'ultimoid':ultimoidclienti,'successful_submit': True}	)
	if interessato.is_valid():
		interessato.save()
		return render(request, 'nuovocliente.html',{"clienti":clienti, "interessato":interessato, 'ultimoid':ultimoidclienti,'successful_submit_interessati': True}	)
