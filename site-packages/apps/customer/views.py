...
from items.models import InvoiceItem,InvoiceItem, offertainternet, cadenzapagamento, metodopagamento
...
def nuovocliente(request):
	...
	ultimoidclienti=Customer.objects.latest('id','customer_id')
	if clienti.is_valid():
		clienti.save()
		return render(request, 'nuovocliente.html', context={'clienti': clientiForm(), 'ultimoid':ultimoidclienti})
	if interessato.is_valid():
		interessato.save()
		return render(request, 'nuovocliente.html', context={'interessato': interessatiForm(),'ultimoid':ultimoidclienti})
