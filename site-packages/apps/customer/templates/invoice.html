{% extends 'base.html' %}

{% block content %}

<style type="text/css">
  .btn-purple, .btn-purple:hover, .btn-purple:active, .btn-purple:visited {
    background-color: #800080 !important;
    color: #ffffff;
}
</style>
<style> 

.colorefootable {
  background-color: #f7f7f7 ;
  color: #000000;
  font-size: 100%;
}


.colorebodytable {
  background-color: #5cb85c   ;
  color: #000000;
  font-size: 100%;
}


#prezzo_subtot {
 color: black;
}

#prezzo_subtot::placeholder {
  color: black;
}

#total {
  color:black;
}

#total::placeholder {
  color: black;
}

input. {
  background: white;
  outline: none;
  border: none;
}

</style>
{% if error_message %}

<div class="alert alert-danger">{{ message }}</div>

{% endif %}

<div class="content-wrapper">
  <section class="content-header">
    <div class="container-fluid">


{% if invoice %}

  <!-- Content Wrapper. Contains page content. Contiene intestazione   -->

      </div><!-- /.container-fluid -->
    </section>

    <section class="content">
      <div class="container-fluid">
        <div class="card card-primary">
    <div class="card-header">

  <h2 class="card-title"><a id=tipo_documento>{{invoice.documento}}</a> n. {{invoice.invoice_id}}</h2>
</div>

<div class="card-body">
        <form action="" method="post" id="prodottiinfattura">
        {% csrf_token %}
        <div class="row">
          <div class="col-12">
            <div class="callout callout-info">
              <h5><i class="fas fa-info"></i> Nota:</h5>
              Quando si invierà la fattura, l'unico modo per annullarla è effettuare una nota di credito.
            </div>
          </div>
        </div>


            <!-- Main content -->
            <div class="invoice p-3 mb-3">
              <!-- title row -->
              <div class="row">
                <div class="col-8">
                  <h4><i><img alt="logo_cliente" src="/static/images/logocliente.png" width="80px"/></i></h4>
                   </div>
                   <br>
                   <br>
                   <br>
                   <br>
                </div>
                <br>
                <!-- /.row -->
                <br>
            
              <!-- info row -->
              <div class="row invoice-info">
                <div class="col-sm-4 invoice-col">
                  Emesso da:
                  <address>
                    <strong>Arpanet Italia s.r.l,</strong><br>
                    <strong>Adress:</strong>Via Caldera 21<br>
                    <strong>City.</strong>Milano, Mi<br>
                    <strong>zip.</strong>20153,&nbsp;Italy<br>
                    <strong>Tel:</strong>&nbsp; (804) 123-5432<br>
                    <strong>P.iva:</strong>&nbsp; It0121234551<br>
                    <strong>Email:</strong>&nbsp; info@arapnetitalia.com
                  </address>
                </div>
                <!-- /.col -->
                <div class="col-sm-4 invoice-col">
                  Cliente:
                  <address>
                    <strong>Customer:</strong>&nbsp;&nbsp;{{invoice.customer.ragionesociale}}<br>
                    <strong>adress:</strong> &nbsp;{{invoice.customer.indirizzo_res}}<br>
                   <strong>city:</strong> &nbsp;{{invoice.customer.comune_res}},&nbsp;&nbsp;{{invoice.customer.prov_res}},<br><strong>zip:&nbsp;</strong>{{invoice.customer.cap_res}},&nbsp; Italy<br>
                    <strong>tel:</strong>&nbsp;{{invoice.customer.recapito_tel}},&nbsp;&nbsp; <strong>Fax:&nbsp;</strong>{{invoice.customer.fax|default_if_none:''}}
                    <br>
                    {% if invoice.customer.piva %}
                     <strong>vat:</strong> &nbsp;{{invoice.customer.piva}},&nbsp;&nbsp;<br>
                     {% else %}
                     <strong>Tax id:</strong> &nbsp;<strong> <font color="#152238">{{invoice.customer.codicefiscale}},&nbsp;&nbsp;</font></strong><br>
                     {% endif %}
                    <a id="email_cliente" value="{{invoice.customer.email}}"><strong>Email:&nbsp;</strong>{{invoice.customer.email}}</a>  
                    <a id="pec_cliente" value="{{invoice.customer.pec}}">, <strong>Pec:&nbsp;</strong>{{invoice.customer.pec}}</a>
                  </address>
                </div>
                <!-- /.col -->
                <div class="col-sm-4 invoice-col">
                  <h4><b>{{invoice.documento}} numero. {{invoice.invoice_id}} </b><br></h4>
               <div class="form-group form-inline"> 
         <label ><font color="#0275d8">Data emissione:&nbsp;</font></label>     
        <input type="text" class="form-control  font-weight-bold " name="invoicedate" id="invoicedate" value="{{ invoice.date|date:'d-m-Y' }}" size="6">
        </div>
                  <div class="form-group form-inline" id="scadenza_documento">
              <label class="col-xs-3 control-label" id="titolo_scadenza_documento"><font color="red">Scadenza fattura:</font></label>&nbsp;
               <div class="col-xs-2">
        <select id="selezionascadenza" class="form-control  font-weight-bold text-danger" style="width:auto;"><option value="0">Data emissione documento</option><option value="15 ">15 giorni emissione documento</option><option selected value="30">30 giorni emmissione documento</option><option value="60">60 giorni emissione documento</option><option value="90">90 giorni emissione documento</option></select>
      </div>
    </div>
      </div>
                <!-- /.col -->
    </div>
              <!-- /.row -->
<br>
<br>
<br>
<br>
<!-- Table row -->
<div class="row">
  <div class="col-12 table-responsive">
      
    <table  id="tbl_posts" class="table table-stripe">
      <thead>
      <tr>
        <th>N.</th>
        <th>Prodotto</th>
        <th>Descrizione</th>
        <th>Quantità</th>
        <th>Prezzo</th>
        <th>Prezzo sub.totale</th>
        <th>Tax</th>
        <th>Totale</th>
        <th>Elimina</th>
    </tr>
  </thead>


<tbody id="tbl_posts_body" >
   
      <tr id="rec-1" class="tr_tabella colorebodytable">
        <td><span class="sn">1</span>.</td>
        <td><input class="form-control prodotto" type="text" size="30"></td>
        <td><textarea class="md-textarea form-control descrizione" rows="1"></textarea></td>
        <td><input type="number" size="7" class="quantita form-control"></td>
        <td><input type="number" step="0.01" size="7" class="prezzo form-control"></td>
        <td><input type="number" step="0.01" size="7" disabled class="prezzo_sub_tot form-control"></td>
        <td><input type="number" size="7" class="tax form-control"></td>
        <td><input type="number" step="0.01" size="7" disabled class="totale form-control"></td>
        <td><a class="btn btn-xs delete-record" data-id="1"><i><img alt="archivia" src="/static/images/cestino.svg" width="32px" title="elimina riga"></i></a></td>
     </tr>

      
</tbody>
 </table>
   

  </div>

</div>
                <!-- /.col -->
              <!-- /.row -->           <!--aggiunge crd per le righe della fattura-->


<div class="row">
    <div class="col-sm-2">
    <label>Aggiungi riga</label>
    <div class="well clearfix">
        <a class="btn  pull-right add-record" data-added="0"><i><img alt="aggiungi riga" src="/static/images/aggiungi.svg" width="50px" title="aggiungi riga"></i></a>
      </div>
    </div>
  </div>
    <div class="row">
    <div class="col-sm-12">
      <label>Note</label>
      <textarea class='md-textarea form-control note' cols="330" id='note' rows='2'></textarea>
    </div>
  </div>


<br>
<br>

<div class="row">
<div class="col-sm-4">
  <h5>Altre voci fattura</h5>
</div>
</div>

<div class="row">
<div class="col-12 table-responsive">
  <table class="table table-stripe">

  <tfoot>
    <tr>
    <th>Sconto</th>
    <th>Spese spedizione</th>
    <th>Altri oneri</th>
    <th>Descrizione altri oneri</th>
</tr>
<tr class="colorefootable">  
    <td><input type="number" value="0.00" class="sconto form-control" /> </td>
<td><input type="number"   value="0.00" class="spese_spedizione form-control"/></td>

<td><input type="number" id="descrizioneoneri"   value="0.00" class="altri_oneri_fattura form-control"/></td> 
<td id="descrizioneoneri"><textarea class='md-textarea form-control altri_oneri_descrizione' id='altri_oneri_descrizione'  value="Non dichiarato" disabled="true" cols='100' rows='2'></textarea></td> 


</tr>
</tfoot>


</table>


</div>
</div>


<br>
<br>
<br>
<br>
  <div class="row">
        <!-- accepted payments column -->
        <div class="col-6 ">
          <p class="lead">Metodi di pagamento:</p>
          <img src="/static/images/visa.png" alt="Visa">
          <img src="/static/images/mastercard.png" alt="Mastercard">
          <img src="/static/images/american-express.png" alt="American Express">
          <img src="/static/images/paypal2.png" alt="Paypal">
          <p class="text-muted well well-sm shadow-none" style="margin-top: 10px;">
            Il pagamento dovrà effettuarsi con i metodi sopra elencati, salvo che non sia previsto come metodo di pagamento l'accredito in un conto corrente
          </p>
        </div>
        
        <!-- /.col -->
        <div class="col-4 ml-auto">
          <h5 class="lead text-danger" >Scadenza fattura <p class="lead text-danger" id="scadenza"></p></h5>
          <div class="table-responsive">
            <table class="table">
              <tr>
                <th style="width:50%">Totale tasse escl.</th>
                <td id="tasse_escluse">€0.00</td>
              </tr>
              <tr>
                <th>Iva</th>
                <td id="tot_iva">€ 0.00</td>
              </tr>
              <tr>
                <th>Costo spedizione</th>
                <td id="cost_spedizioni">€ 0.00</td>
              </tr>
              <tr>
                <th>Altri oneri</th>
                <td id="altri_oneri">€ 0.00</td>
              </tr>
              <tr>
                <th>Totale:</th>
                <td id="totale_della_fattura"><strong>€ 0.00</strong></td>
              </tr>
            </table>
          </div>
                </div>
                <!-- /.col -->
              </div>
              <!-- /.row dlla tabella riepilogo -->

              <!-- this row will not appear when printing -->
              <div class="row no-print">
                <div class="col-12">
                  <a href="invoice-print.html" rel="noopener" target="_blank" class="btn btn-default"><i class="fas fa-print"></i> Print</a>
                  <button type="button" class="btn btn-warning float-right"><i class="far fa-credit-card"></i> Submit Payment
                  </button>

                  <button type="button" class="btn btn-primary float-right" style="margin-right: 5px;">
                    <i class="fas fa-download"></i> Generate PDF
                  </button>
                   <button  value="submit" type="Submit" class="btn btn-purple float-right" id ="send_invoice" style="margin-right: 5px;">
                    <i class="fas fa-file-invoice"></i> Send invoice
                  </button>
                </div>
              </div>
            
            </div>
            <!-- /.invoice -->
          </div><!-- /.col -->
      </div>
    </form>
  </div>
</div>
    </div>
  </section>
</div>
       <!-- /.row -->
    <!-- /.content -->
 





<!-- Delete confirmation modal -->
<div class="modal fade" id="confirmdelete" role="dialog">
  <div class="modal-dialog">
    <div class="modal-content">
      <div class="modal-header">
        <h4 class="modal-title">Confirm</h4>
      </div>
      <div class="modal-body">

      </div>
      <div class="modal-footer">
        <button type="button" class="btn btn-default" data-dismiss="modal">No</button>
        <a href="#" class="btn btn-danger">Yes</a>
      </div>
    </div>
  </div>
</div>




  
<div style="display:none;">
    <table id="sample_table">
       <tr id="" class="tr_tabella colorebodytable">
        <td><span class="sn"></span>.</td>
       <td><input class="form-control prodotto" type="text" size="30"></td>
        <td><textarea class="md-textarea form-control descrizione" rows="1"></textarea></td>
        <td><input type="number" size="7" class="quantita form-control"></td>
        <td><input type="number" size="7" class="prezzo form-control"></td>
        <td><input type="number" size="7" disabled class="prezzo_sub_tot form-control"></td>
        <td><input type="number" size="7" class="tax form-control"></td>
        <td><input type="number" size="7" disabled class="totale form-control"></td>
        <td><a class="btn btn-xs delete-record" data-id="1"><i><img alt="elimina" src="/static/images/cestino.svg" width="32px" title="elimina riga"></i></a></td>
     </tr>
   </table>
 </div>

<script>
  $(document).ready(function() {
    $('#confirmdelete').on('show.bs.modal', function(e) {
      var name = $(e.relatedTarget).data('object-name');
      var url = $(e.relatedTarget).data('delete-url');
      $('#confirmdelete .modal-body').text("Are you sure you want to delete " + name + "?");
      $('#confirmdelete .modal-footer a').attr('href', url);
    });
  });
</script>


<script type="text/javascript">//serve per rendereitaliano il calendario
          $(function () {
            $('#invoicedate').datepicker({
odayBtn: "linked",
autoclose: true,
closeText: 'Chiudi',
prevText: 'Prec',
nextText: 'Succ',
currentText: 'Oggi',
monthNames: ['Gennaio','Febbraio','Marzo','Aprile','Maggio','Giugno', 'Luglio','Agosto','Settembre','Ottobre','Novembre','Dicembre'],
monthNamesShort: ['Gen','Feb','Mar','Apr','Mag','Giu', 'Lug','Ago','Set','Ott','Nov','Dic'],
dayNames: ['Domenica','Lunedì','Martedì','Mercoledì','Giovedì','Venerdì','Sabato'],
dayNamesShort: ['Dom','Lun','Mar','Mer','Gio','Ven','Sab'],
dayNamesMin: ['Do','Lu','Ma','Me','Gio','Ve','Sa'],
dateFormat: 'yy-mm-dd',
firstDay: 1,
isRTL: false
});

$.datepicker.setDefaults($.datepicker.regional['it']);
} );
</script>



<script type="text/javascript">
  $(function() {
  var tipo_documento=$('#tipo_documento').text();
 
  if(tipo_documento=="Nota di credito"){
    $('#scadenza_documento').hide()
  }
  else if(tipo_documento=="Preventivo"){
    $('#titolo_scadenza_documento').text('Scadenza preventivo')
     $('#titolo_scadenza_documento').css("color","red")
   
  };

 })
</script>




<!--script per i prodotti fattura-->



  <!-- per la tabella dinamica fattura-->

  <script type="text/javascript">

jQuery(document).delegate('a.add-record', 'click', function(e) {
     e.preventDefault();    
     var content = jQuery('#sample_table tr'),
     size = jQuery('#tbl_posts >tbody >tr').length + 1,
     element = null,    
     element = content.clone();
     element.attr('id', 'rec-'+size);
     element.find('.delete-record').attr('data-id', size);
     element.appendTo('#tbl_posts_body');
     element.find('.sn').html(size);

});


jQuery(document).delegate('a.delete-record', 'click', function(e) {
     e.preventDefault();    
     var didConfirm = confirm("Sei sicuro di volere eleminare questa riga?");
     if (didConfirm == true) {
      var id = jQuery(this).attr('data-id');
      var targetDiv = jQuery(this).attr('targetDiv');
      jQuery('#rec-' + id).remove();
      // se si premuto il puilsante di eliminare la riga deve aggiornare la tabella
      //per la somma di tutto il totale
    var totaleconiva = 0;
    $(".totale").each(function(){
        totaleconiva += +$(this).val();
    });
    var spesedispedizione=$('.spese_spedizione ').val()
    var altrioneri=$('.altri_oneri_fattura ').val()
    var sum= +totaleconiva + +spesedispedizione + +altrioneri //il più davanti trasforma da valore stringa a numero

    $("#totale_della_fattura").text(sum)
    $("#cost_spedizioni").text(spesedispedizione)
    $("#altri_oneri").text(altrioneri)
    

  
  //per la somma del sub totale
  var quotasubtot = 0;
  $(".prezzo_sub_tot").each(function(){
        quotasubtot += +$(this).val();
    });
    $("#tasse_escluse").text(quotasubtot)

// per la somma di tutta l'iva
     var totaleiva=totaleconiva-quotasubtot
     $("#tot_iva").text(totaleiva)

      
    //regnerate index number on table
    $('#tbl_posts_body tr').each(function(index) {
      //alert(index);
      $(this).find('span.sn').html(index+1);
    });
    return true;
  } else {
    return false;
  }
});

</script>
<script type="text/javascript">
  $(document).ready(function(){
  $('.altri_oneri_fattura ').on('input',function(){
  
var altrioneri_campo_nascosto=$('.altri_oneri_fattura ').val()

console.log(altrioneri_campo_nascosto)
if(altrioneri_campo_nascosto > 0  ){
  
  $(' #altri_oneri_descrizione').prop('disabled',false);
  $(' #label_altrioneri').prop('disabled',false);

}

else{
  $(' #altri_oneri_descrizione').prop('disabled',true);
  $(' #label_altrioneri').prop('disabled',true);

};

});
});
</script>

<script type="text/javascript">//per i calcoli fattura
$(document).ready(function(){



$('table').on('keyup', 'input', function(){ // run anytime the value changes
    $thisRow = $(this).parent().parent();
    var quantita  = Number($thisRow.find('td > .quantita').val());   // get value of field
    var prezzo = Number($thisRow.find('td > .prezzo').val()); // convert it to a float
    var iva = Number($thisRow.find('td > .tax').val());
    $thisRow.find('td>.totale').val('');
    $thisRow.find('td>.prezzo_sub_tot').val('');
    var subtotale=prezzo*quantita
    var percento=(subtotale*iva)/100
    var totale=subtotale+percento
    $thisRow.find('td>.totale').val(totale);
    $thisRow.find('td>.prezzo_sub_tot').val(subtotale);
    
   
    //per la somma di tutto il totale
    var totaleconiva = 0;
    $(".totale").each(function(){
        totaleconiva += +$(this).val();
    });
    var spesedispedizione=$('.spese_spedizione ').val()
    var altrioneri=$('.altri_oneri_fattura ').val()
    var sum= +totaleconiva + +spesedispedizione + +altrioneri //il più davanti trasforma da valore stringa a numero
    console.log(aggiunnta_riassunto_altrioneri)
    $("#totale_della_fattura").text(sum)
    $("#cost_spedizioni").text(spesedispedizione)
    var onere_descrizione=altrioneri+aggiunnta_riassunto_altrioneri
    $("#altri_oneri").text(onere_descrizione)
    

  
  //per la somma del sub totale
  var quotasubtot = 0;
  $(".prezzo_sub_tot").each(function(){
        quotasubtot += +$(this).val();
    });
    $("#tasse_escluse").text(quotasubtot)

// per la somma di tutta l'iva
     var totaleiva=totaleconiva-quotasubtot
     $("#tot_iva").text(totaleiva)





  });
$("#tbl_posts").click(function(){
  //per la somma di tutto il totale
    var totaleconiva = 0;
    $(".totale").each(function(){
        totaleconiva += +$(this).val();
    });
    var spesedispedizione=$('.spese_spedizione ').val()
    var altrioneri=$('.altri_oneri_fattura ').val()
    var sum= +totaleconiva + +spesedispedizione + +altrioneri //il più davanti trasforma da valore stringa a numero

    $("#totale_della_fattura").text(sum)
    $("#cost_spedizioni").text(spesedispedizione)
    $("#altri_oneri").text(altrioneri)
    

  
  //per la somma del sub totale
  var quotasubtot = 0;
  $(".prezzo_sub_tot").each(function(){
        quotasubtot += +$(this).val();
    });
    $("#tasse_escluse").text(quotasubtot)

// per la somma di tutta l'iva
     var totaleiva=totaleconiva-quotasubtot
     $("#tot_iva").text(totaleiva)


 
});
});

</script>
<script type="text/javascript">
  //scadenza fattura

  $('#selezionascadenza').change(function(){
    var selezione_scadenza=$('#selezionascadenza').val();
    var options = { year: 'numeric', month: 'numeric', day: 'numeric' };//se si usa long(al posto del numeric si vedrà il nome corrispondente), si può mettere weekday per sapere il giorno della data
    console.log(selezione_scadenza)
    var scadenza=new Date();
    console.log(scadenza)
    scadenza.setDate(scadenza.getDate() + +selezione_scadenza);
    var scadenza_da_mostrare=scadenza
    var a=scadenza.toLocaleDateString("it-IT", options);
    console.log(scadenza_da_mostrare)
    $('#scadenza').text(a)

  })
</script>



<!-- ./wrapper -->
{% endif %}

{% endblock %}