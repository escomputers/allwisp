...
<div class="col-xl-12">
  ...
  <h1 class="card-title">Lista fatture</h1>
</div>
<div class="card-body">
...
<div class="row">
	<div class="col-sm-6">
          <h4>Crea documento finanziario<a type="button"  class="btn btn-sm " href="{% url 'new_invoice' %}"><i><img alt="aggiungi-clienti" src="/static/images/piu.svg" width="40px" title="Nuovo cliente"></i></a></h4>
      </div>
</div>
...
<div class="col-sm-12">
  ...
  <h1 class="card-title">Riepilogo economico fatture</h1>
</div>
<div class="card-body">  
        ...
        <div class="row">
        	 <div class="col-sm-2">
        	 <label>Ultimo id fattura:</label>
        	<h5><font color="#ff4d01">{{ ultimoid }} </font></h5>
        </div>

        <div class="col-sm-1">
          <label>Fatture tot a.c</label>
          <h5> <font color="#ff4d01">{{ lista.count }} </font></h5><!--conta il numero degli oggetit-->
        </div>
       
         <div class="col-sm-1">
          <label>Fatture scadute</label>
          <h5> <font color="#ff4d01">{{ fatture_scadute.count }} </font></h5>
        </div>

        <div class="col-sm-2" >
          <label >Da riscuotere</label>
          <h5> <font color="#ff4d01">{{ da_riscuotere }} </font></h5>
         
        </div>
        <div class="col-sm-2" >
          <label>Tot.incassato a.c</label>
          <h5> <B><font color="#006400">€{{ incassato }} </font></B></h5>
         
        </div>
        <div class="col-sm-2" >
          <label>Tot.da riscuotere</label>
          <h5> <B><font color="#f0ad4e  ">€{{ soldi_da_ricevere|default_if_none:"0" }} </font></B></h5>
         
        </div>
        <div class="col-sm-2" >
          <label>Tot.fatt.scadute</label>
          <h5 id='tot_fatture_scadute'> <B><font color="#B30000">€{{ tot_fatture_scadute|default_if_none:"0.00" }} </font></B></h5>
      </div>
</div>
      <br>
      <br>
  ...
  <button class="btn btn-danger" type="button" data-toggle="collapse" data-target="#collapseExample" aria-expanded="false" aria-controls="collapseExample">
   I 10 peggiori clienti
  </button>
</p>
<div class="collapse" id="collapseExample">
  <div class="card card-body">
    <h4>Qui trovi la lissta relativa ai clienti selezionati <a id='qualità'>migliori</a></h4>
  </div>
</div>
...
<div class="row">
        <div class="col-sm-5">
           <div> <a title="Maggiori dettagli finaziari" href="#" target="_blank" ><B>
      Clicca qui per maggiori dettagli finanziari</B></a></div>
        </div>
        <div class="col-sm-4">
           <p><B><font color="#5bc0de">* a.c si intende corrente anno</font></B></p>
        </div>
...
<div class="col-sm-12">
  ...
  <h1 class="card-title">Lista delle fatture</h1>
</div>
<div class="card-body">  
	...
	<table  id="tabellaclienti"  class="table-responsive cell-border compact row-border stripe " style="width:50%">
		...
		<tr>
			...
			<td class="text-center ragionesociale"><B>{{ liste.customer.ragionesociale }}</B></td>
			{% if liste.customer.piva %}
			<td class="text-center">{{liste.customer.piva}}</td>
			{% else %}
			<td class="text-center">C.F_  {{liste.customer.codicefiscale|default_if_none:"Nessuna"}}</td>
			{% endif %}
			<td class="text-center">{{ liste.date }}</td>
			<td class="text-center">{{ liste.scadenza_fattura }}</td>
			<td class="text-center">{{ liste.totale_fattura }}</td>

			<td class="text-center status">{{ liste.status|default_if_none:'da pagare' }}</td>
			<td class="text-center">{{ liste.customer.email|default_if_none:'Non fornito' }}</td>
			<td class="text-center">{{ liste.customer.pec|default_if_none:'Non fornito' }}</td>
			<td class="text-center">{{ liste.customer.recapito_tel|default_if_none:'Non fornito' }}</td>
			<td class="text-right">
				<div class="btn-group" role="group">
				<a href="{% url 'view_invoice' liste.invoice_id %}" class="btn btn-sm"><i><img alt="dettagli-clienti" src="/static/images/find-person.svg" width="32px" title="Dettagli"></i></a>
