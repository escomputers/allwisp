	...
	<div class="panel panel-primary">
		<div class="panel-body">
			<div class="form-group">
				...
				<div class="col-sm-6">
					<p class="form-control-static"><a href="{% url 'update' invoice.customer.id %}">{{ invoice.customer }}</a></p>
				</div>
			</div>
			<div class="form-group">
			...
			<div class="form-group">
				...
				<div class="col-sm-6">
					<select name="status" class="form-control">
						<option value="Draft" {% if invoice.draft %}selected{% endif %}>Preventivo</option>
						<option value="Paid" {% if invoice.paid %}selected{% endif %}>Pagato</option>
						<option value="Unpaid" {% if invoice.unpaid %}selected{% endif %}>Non pagato</option>
					</select>
				</div>
			</div>
		</div>
		<div class="panel-footer">
			...
			<a href="{% url 'print_invoice' invoice.id %}" target="_new" class="btn btn-default">Stampa fattura</a>
		</div>
	</div>
</form>
...
<form action="{% url 'add_item' invoice.id %}" method="post" class="form-inline">
	{% csrf_token %}
	<div class="panel panel-primary">
		<div class="panel-heading">Prodotti fattura</div>
		<table class="table table-hover">
			...
			<tr>
				...
				<td>{{ item.qty }}</td>
				<td class="text-right">
					<button type="button" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#confirmdelete" data-object-name="{{ item.description }}" data-delete-url="{% url 'delete_item' invoice.id item.id %}">Elimina</button>
				</td>
			</tr>
			{% endfor %}
			<tr>
		...
		<div class="panel-footer text-right"><strong>Totale: €{{ invoice.total_items }}</strong></div>
	</div>
</form>
...
<form action="{% url 'add_invoice_expense' invoice.id %}" method="post" class="form-inline">
	{% csrf_token %}
	<div class="panel panel-primary">
		<div class="panel-heading">Costi per fattura</div>
		<table class="table table-hover">
			<tr>
				<th>Descrizione</th>
				<th>Costi</th>
			...
			<tr>
				...
				<td>${{ expense.total }}</td>
				<td class="text-right">
					<button type="button" class="btn btn-xs btn-danger" data-toggle="modal" data-target="#confirmdelete" data-object-name="{{ expense.description }}" data-delete-url="{% url 'delete_invoice_expense' invoice.id expense.id %}">Elimina</button>
				</td>
			</tr>
			{% endfor %}
			<tr>
		...
		<div class="panel-footer text-right"><strong>Totale: €{{ invoice.total_expenses }}</strong></div>
	</div>
</form>
	...
	<div class="panel panel-primary">
		...
		<table class="table table-hover">
			...
			<tr>
				...
				<td class="text-right">
					<button type="button" data-toggle="modal" data-target="#confirmdelete" data-object-name="{{ attachment.displayname }}" data-delete-url="{% url 'delete_invoice_attachment' invoice.id attachment.id %}" class="btn btn-xs btn-danger">Elimina</button>
				</td>
			</tr>
			{% endfor %}
			<tr>
...
<div class="well text-right">Totale netto fattura: €{{ invoice.total }}</div>
...
<div class="alert alert-warning">Nuemro fattura non trovata!</div>
